...
public class JoystickView extends View {
	...
	public final static int CONSTRAIN_BOX = 1;
	public final static int CONSTRAIN_CIRCLE = 0;
	
	public String contrainteType = "CIRCLE"; // Box Horisontal Vertical CIRCLE
...
//		rect.set(65, 0, 105, 170);
		
		
		
		
		
//		rect.set((getMeasuredHeight()/2-20), 0, (getMeasuredHeight()/2+20), getMeasuredWidth());
		
		if (contrainteType == "CIRCLE") {
			canvas.drawCircle(cX, cY, bgRadius, bgPaint);
			
		} else if (contrainteType == "BOX") {
			canvas.drawCircle(cX, cY, bgRadius, bgPaint);
			
		} else if (contrainteType == "Horisontal") {
			float border = handleRadius/2;
			rect.set(border, (getMeasuredWidth()/2-handleRadius), getMeasuredHeight()-border , (getMeasuredWidth()/2+handleRadius));
			canvas.drawRoundRect(rect, 5, 5, bgPaint);
			
		} else if (contrainteType == "Vertical") {
			float border = handleRadius/2;
			
			rect.set((getMeasuredHeight()/2-handleRadius), border, (getMeasuredHeight()/2+handleRadius), getMeasuredWidth()-border);
		...
		if (D) {
			...
			canvas.drawCircle(handleX, handleY, 1, dbgPaint1);
...
//				Log.d(TAG, String.format("moveListener.OnMoved(%d,%d)", (int)userX, (int)userY));
				moveListener.OnMoved( (int)userX,  (int) Math.round(userY*-1), (int) Math.min(Math.ceil(radial),innerPadding));
			}
		}
	}
	...
	private void calcUserCoordinates() {
		...
		int movementRange =  (int) Math.round(this.movementRange);
		cartX = (int)(Math.round(touchX / movementRadius * movementRange));
		cartY = (int)(Math.round(touchY /movementRadius * movementRange));
