...
class Servo(object):
   ...
   def __init__(self, directionGPIO, directionMinGauche, directionMaxDroite):
      try:
         ...
         print "Err [Init Servo] : ", e

   def __del__(self):
      PWM.clear_channel_gpio(0, self.directionGPIO)
      PWM.cleanup()
...
class VoiTuxServer(object):
   def __init__(self):
      try:
         ...
         directionGPIO            = 4   # Servo Number 0 = GPIO 4 = PIN 7
         self.directionMinGauche  = 130  # Valeur min (buté gauche)
         self.directionMaxDroite  = 190 # Valeur max (buté droite)
...
# os.system("raspivid -t 0 -w 640 -h 360 -fps 15 -b 1200000 -p 0,0,640,480 -o - | " 
# "gst-launch -v fdsrc ! h264parse ! rtph264pay config-interval=1 pt=96 ! gdppay ! tcpserversink host=192.168.0.23 port=5000 &")

# os.system("raspivid -t 999999 -w 640 -h 360 -fps 15 -b 2000000 -p 0,0,640,480 -o - | " 
# "gst-launch-1.0 -v fdsrc ! h264parse ! rtph264pay config-interval=1 pt=96 ! gdppay ! tcpserversink host=192.168.0.19 port=5000 &")
